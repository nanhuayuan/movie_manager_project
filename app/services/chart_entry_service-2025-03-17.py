...
class ChartEntryService(BaseService[ChartEntry, ChartEntryDAO]):
    ...
    def get_by_chart_and_actor(self, chart_id: int, actor_id: int) -> Optional[ChartEntry]:
        ...
        try:
            # 定义 criteria 字典
            criteria = {'actor_id': actor_id, 'chart_id': chart_id}
            ...
            # 检查结果
            if chart_entry_list and len(chart_entry_list) > 0:
                info(f"Chart entry found for actor_id: {actor_id} and chart_id: {chart_id}")
                return chart_entry_list[0]
            else:
                info(f"No chart entry found for actor_id: {actor_id} and chart_id: {chart_id}")
                return None
        except ValueError as e:
            error(f"ValueError: {e}")
            return None

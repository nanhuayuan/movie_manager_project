...
from app.utils.log_util import info
...
class ChartService:
    ...
    def get_base_path(self, base_path) -> List[MdFileInfo]:
    ...
    def get_movie_chart(chart_type: ChartType,
                        path: str = "../data/movies_list") -> List[MdFileInfo]:
        """
        获取电影榜单的便捷函数
    ...
    def get_movie_chart_and_chart_type(self, chart_type: ChartType = None) -> Tuple[List[MdFileInfo], ChartType]:
        """
           ...
           :param chart_type: 榜单类型，默认为折线图
           :return: 包含榜单数据的列表和榜单类型
           """
    ...
    def read_file_to_db(self):
        ...
        for md_file in md_file_list:
            ...
            for movie in md_file.movie_info_list:
                ...
                chart_entry = self.movie_to_chart_entry(movie=movie)

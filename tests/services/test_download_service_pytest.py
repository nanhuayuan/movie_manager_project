...
def mock_service(mock_config):
    """mock下载服务"""
    with patch('app.config.app_config.AppConfig') as mock_app_config:
        mock_app_config().get_download_client_config.return_value = mock_config
        service = DownloadService()
        service.client = Mock()
        service.client.connect.return_value = True
        return service
...
class TestDownloadService:
    ...
    def test_init(self, mock_config):
        ...
        with patch('app.config.app_config.AppConfig') as mock_app_config:
            mock_app_config().get_download_client_config.return_value = mock_config
            service = DownloadService()
            assert service.host == mock_config['host']
            assert service.port == mock_config['port']
            assert service.username == mock_config['username']
            assert service.password == mock_config['password']
            assert service.client_type == mock_config['type']
            assert service.speed_limit == 7 * 1024 * 1024  # 7MB/s

    def test_add_download(self, mock_service):
        """测试添加下载"""
        mock_service.client.add_torrent.return_value = True
        magnet = 'magnet:?xt=test'
        save_path = '/downloads'

        result = mock_service.add_download(magnet, save_path)

        assert result is True
        mock_service.client.add_torrent.assert_called_once_with(magnet, save_path)
